        ...
        if (declaration.value.propertyId.property === "transform") {
            for (const transform of declaration.value.value) {
                if (transform.value.name === "translate") {
                    addTransformProp("translateX", parseUnparsed(transform.value.arguments[0], parseOptions));
                    addTransformProp("translateY", parseUnparsed(transform.value.arguments[2], parseOptions));
                }
                else {
                    addTransformProp(transform.value.name, parseUnparsed(transform.value.arguments, parseOptions));
                }
            }
            return;
        }
...
function parseCalc(args, options) {
    const parsed = [];
    let mode;
